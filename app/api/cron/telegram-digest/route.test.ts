...
const mocks = vi.hoisted(() => ({ publish: vi.fn(), auth: vi.fn(), cron: vi.fn(async (_job: string, execute: () => Promise<unknown>) => execute()) }));
vi.mock("@/lib/telegram", () => ({ publishTelegramMorning: mocks.publish }));
vi.mock("@/lib/operations", () => ({ withCronRun: mocks.cron }));
...
describe("telegram digest cron", () => {
  ...
  it("supports protected forced dry runs and records the attempt", async () => { const response = await GET(new Request("http://local/api/cron/telegram-digest?dryRun=1&force=1")); expect(response.status).toBe(200); expect(mocks.publish).toHaveBeenCalledWith(expect.any(Date), { dryRun: true, force: true }); expect(mocks.cron).toHaveBeenCalledWith("telegram-digest", expect.any(Function)); });
});
