...
import { sendDirectorNotifications, sendKickoffReminders } from "@/lib/push";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const previousSuccessfulAt = typeof previous?.payload === "object" && previous.payload !== null &&
      ...
      : null;
    const succeeded = stats.sent + stats.checkedFixtures + director.sent;
    if (!(stats.errors > 0 && succeeded === 0)) {
      await prisma.apiCache.upsert({
        where: { key: healthKey },
        create: {
          key: healthKey,
          payload: { completedAt: completedAt.toISOString(), ...stats, director },
    ...
    const result = { ...stats, director, completedAt: completedAt.toISOString(), previousSuccessfulAt };
