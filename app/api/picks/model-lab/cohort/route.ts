...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      rows: page.map((row) => { const result = byFixture.get(row.fixtureId); const hit = binaryOutcome(row.market, row.side, result?.homeGoals ?? null, result?.awayGoals ?? null, row.line); const close = freshClosing(row.kickoff, row.closedAt, row.closingMarketProbability).close; return { ...row, kickoff: row.kickoff.toISOString(), qualifiedAt: row.qualifiedAt?.toISOString() ?? null, closedAt: row.closedAt?.toISOString() ?? null, homeGoals: result?.homeGoals ?? null, awayGoals: result?.awayGoals ?? null, resultStatus: result?.status ?? null, hit, profit: hit == null || row.decimalOdds == null ? null : hit ? row.stake * (row.decimalOdds - 1) : -row.stake, freshClosingProbability: close, clv: close == null ? null : close - row.marketProbability }; }),
