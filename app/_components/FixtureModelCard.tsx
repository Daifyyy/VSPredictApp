...
function ExpectedMatch({ forecast }: { forecast: FixtureModelForecast }) {
  ...
  return <section className="rounded-xl border border-border bg-surface p-3" aria-label="Očekávaný obraz zápasu">
    ...
    <div className="mt-3 flex h-2.5 overflow-hidden rounded-full bg-border" aria-label={`Domácí ${pct(forecast.outcome.home)}, remíza ${pct(forecast.outcome.draw)}, hosté ${pct(forecast.outcome.away)}`}><span className="bg-home" style={{ width: `${forecast.outcome.home / total * 100}%` }} /><span className="bg-warning" style={{ width: `${forecast.outcome.draw / total * 100}%` }} /><span className="bg-[#7c3aed]" style={{ width: `${forecast.outcome.away / total * 100}%` }} /></div>
...
function PerformancePressure({ forecast }: { forecast: FixtureModelForecast }) {
  ...
  const side = (label: string, data: typeof shadow.home) => <div className="rounded-lg bg-background px-3 py-3"><strong className="text-foreground">{label}</strong><dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 tabular-nums"><MiniDatum label="Index oček. tlaku" value={value(data.expectedPressure, 0)} /><MiniDatum label="Schopnost tlaku" value={value(data.pressureCapability, 0, "/100")} /><MiniDatum label="Střely" value={value(data.shots, 1)} /><MiniDatum label="Na branku" value={value(data.shotsOnTarget, 1)} /><MiniDatum label="Z vápna" value={value(data.shotsInsideBox, 1)} /><MiniDatum label="xG / střelu" value={value(data.shotQuality, 2)} /></dl></div>;
  return <section className="rounded-xl border border-border bg-surface p-3" aria-label="Shadow model tlaku a tvorby šancí">
    ...
    <p className="mt-2 text-[10px] text-muted">Index očekávaného tlaku: 100 znamená velmi silný tlak, hodnoty nad 100 extrémně příznivý matchup. Pokrytí vstupů {shadow.coverage.available}/{shadow.coverage.expected}. Heuristická v1 se nejprve sbírá pro chronologický backtest; není kalibrovanou pravděpodobností.</p>
  </section>;
}
