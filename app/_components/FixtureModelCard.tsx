...
function PerformancePressure({ forecast }: { forecast: FixtureModelForecast }) {
  ...
  const totalShots=(shape.home.shots.value??0)+(shape.away.shots.value??0);
  const totalShotsLow=(shape.home.shots.interval?.low??shape.home.shots.value??0)+(shape.away.shots.interval?.low??shape.away.shots.value??0);
  const totalShotsHigh=(shape.home.shots.interval?.high??shape.home.shots.value??0)+(shape.away.shots.interval?.high??shape.away.shots.value??0);
  ...
  return <section className="rounded-xl border border-border bg-surface p-3" aria-label="Shadow model tlaku a tvorby šancí">
    ...
    <div className="mt-2 grid gap-2 sm:grid-cols-4"><Metric label="Očekávaný objem" value={`${totalShots.toFixed(0)} střel · typicky ${totalShotsLow.toFixed(0)}–${totalShotsHigh.toFixed(0)}`} /><Metric label="Současné λ" value={shadow.currentTotalLambda.toFixed(2)} /><Metric label="Shot shadow λ" value={value(shadow.shotTotalLambda, 2)} /><Metric label="Over 2,5" value={`${pct(shadow.currentOver25)} → ${pct(shadow.shadowOver25)}`} /></div>
