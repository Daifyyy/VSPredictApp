...
function QuickCard({ item, focus, open, pro, favorite, onFavorite, onToggle }: { item: QuickItem; focus: QuickFocus; open: boolean; pro: boolean; favorite: boolean; onFavorite: () => void; onToggle: () => void }) {
  ...
  return <article className={`rounded-xl border bg-background ${open ? "border-accent-strong shadow-sm lg:col-span-3" : "border-border"}`}>
    <div className="p-3.5">
      ...
      <div className="mt-3 rounded-lg bg-accent/15 px-3 py-2 text-center text-xs font-bold text-foreground">{item.reason}</div>
      ...
      {item.result && item.audit && <div className="mt-2 text-center text-[10px] text-muted">{item.audit.decimalOdds == null ? "Bez tehdy dostupného kurzu · do ROI nevstupuje" : <>Kurz {item.audit.decimalOdds.toFixed(2)}{item.audit.bookmaker ? ` · ${item.audit.bookmaker}` : ""}{item.audit.profit != null ? ` · ${item.audit.profit >= 0 ? "+" : ""}${item.audit.profit.toFixed(2)} u` : ""}{item.audit.clv != null ? ` · CLV výběru ${item.audit.clv >= 0 ? "+" : ""}${(item.audit.clv * 100).toFixed(1)} p. b.` : ""}</>}</div>}
