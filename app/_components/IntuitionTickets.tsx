...
type Ticket = { slot: number; status: string; combinedOdds: number | null; priceKind: "DIRECT" | "SYNTHETIC" | "NONE"; hit: boolean | null; profit: number | null; legs: Leg[] };
    ...
    <div className={`grid gap-3 ${payload.tickets.length > 1 ? "lg:grid-cols-2" : ""}`}>
      {payload.tickets.map((ticket) => <article key={ticket.slot} className="overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
          ...
          <div className="text-right"><strong className="block tabular-nums text-foreground">{ticket.combinedOdds == null ? "Kurz není dostupný" : `${ticket.priceKind === "SYNTHETIC" ? "Odhad" : "Kurz"} ${ticket.combinedOdds.toFixed(2)}`}</strong>{ticket.hit != null && <span className={`text-[10px] font-bold ${ticket.hit ? "text-positive" : "text-negative"}`}>{ticket.hit ? "TREFENO" : "NETREFENO"}</span>}</div>
        </div>
        <ol className="divide-y divide-border">
          {ticket.legs.map((leg) => <li key={leg.fixtureId} className="px-4 py-3">
            ...
            <div className="mt-1 flex justify-between gap-3 text-[10px] text-muted"><span>{leg.decimalOdds == null ? "Kombinovaný kurz nelze odhadnout" : `${leg.decimalOdds.toFixed(2)} · ${leg.priceKind === "SYNTHETIC" ? "syntetický odhad" : leg.bookmaker}`}</span>{leg.hit != null && <strong className={leg.hit ? "text-positive" : "text-negative"}>{leg.homeGoals}:{leg.awayGoals} · {leg.hit ? "✓" : "✕"}</strong>}</div>
          </li>)}
        </ol>
        <p className="border-t border-border bg-background px-4 py-2 text-[10px] text-muted">Experimentální modelový výběr, nikoli doporučení vkladu. Syntetický kurz zohledňuje modelovou korelaci a je veden odděleně od reálné nabídky.</p>
      </article>)}
    </div>
  </section>;
}
