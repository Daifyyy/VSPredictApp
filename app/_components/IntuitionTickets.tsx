...
type Leg = {
  ...
  bookmaker: string | null; reason: string; eloExpectedValue?: number | null;
  hit: boolean | null; homeGoals: number | null; awayGoals: number | null;
};
type Ticket = {
  slot: number; combinedOdds: number | null; hit: boolean | null; profit: number | null; legs: Leg[];
};
type Block = {
  strategy: "VALUE" | "ELO_INTUITION"; frozen: boolean; tickets: Ticket[]; emptyReason: string | null;
  coverage: { fixtures: number; withOdds: number; candidates: number };
};
...
const emptyText: Record<string, string> = {
  ...
  NOT_ENOUGH_CANDIDATES: "Filtry našly méně než tři kvalifikované příležitosti.",
  NO_VALID_COMBINATION: "Kandidáti existují, ale žádná kombinace nemá 3–4 nohy a kurz 8–30.",
};
...
function StrategyCard({ block }: { block: Block }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
    ...
    {!ticket ? <div className="px-4 py-5"><p className="text-sm text-foreground">{emptyText[block.emptyReason ?? ""] ?? "Tiket pro toto okno nevznikl."}</p><p className="mt-1 text-[11px] text-muted">{block.coverage.withOdds}/{block.coverage.fixtures} zápasů s kurzem · {block.coverage.candidates} kandidátů</p></div> : <>
      ...
      <ol className="divide-y divide-border">{ticket.legs.map((leg) => <li key={leg.fixtureId} className="px-4 py-3">
        ...
        <p className="mt-1 text-[10px] text-muted">{leg.decimalOdds?.toFixed(2)} · {leg.bookmaker}{block.strategy === "ELO_INTUITION" && leg.eloExpectedValue != null ? ` · Elo EV ${(leg.eloExpectedValue * 100).toFixed(1)} %` : ""}</p>
      </li>)}</ol>
