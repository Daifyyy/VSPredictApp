...
export function MatchInsightCard({insight,variant="detail",pro=false}:{insight:MatchInsight;variant?:MatchInsightVariant;pro?:boolean}){
  ...
  const totalShots=expectation?(expectation.home.shots.value??0)+(expectation.away.shots.value??0):null;
  const totalShotsLow=expectation?(expectation.home.shots.interval?.low??expectation.home.shots.value??0)+(expectation.away.shots.interval?.low??expectation.away.shots.value??0):null;
  const totalShotsHigh=expectation?(expectation.home.shots.interval?.high??expectation.home.shots.value??0)+(expectation.away.shots.interval?.high??expectation.away.shots.value??0):null;
...
function range(metric:NonNullable<MatchInsight["expectation"]>["home"]["shots"],digits=0){const value=number(metric.value,digits);return metric.interval?`${value} · typicky ${number(metric.interval.low,digits)}–${number(metric.interval.high,digits)}`:value}
function InsightSide({label,side}:{label:string;side:NonNullable<MatchInsight["expectation"]>["home"]}){return <div className="rounded-lg border border-border p-2"><strong className="text-xs">{label}</strong><dl className="mt-2 grid grid-cols-2 gap-2"><Datum label="xG" value={range(side.xg,2)}/><Datum label="Střely" value={range(side.shots)}/><Datum label="Na branku" value={range(side.shotsOnTarget)}/><Datum label="Z vápna" value={range(side.shotsInsideBox)}/><Datum label="Podíl šancí" value={pct(side.chanceShare.value)}/><Datum label="Držení" value={pct(side.possessionShare.value)}/></dl>{side.shots.warnings?.includes("CLIPPED_OUTLIER")?<p className="mt-2 text-[10px] text-warning">Extrémní vstup byl omezen realistickým kontextovým rozsahem.</p>:null}</div>}
...
function Legend(){return <div className="sm:col-span-2 rounded-lg border border-border px-3 py-2 text-[11px] leading-5 text-muted"><strong className="text-foreground">Jak čísla číst:</strong> „typicky“ je 80% pásmo očekávané variability. Tlak i otevřenost jsou percentily proti srovnatelným zápasům, nikoli pravděpodobnosti. Střely měří objem, xG kvalitu a objem šancí; otevřenost neznamená jistotu gólů.</div>}
const riskLabel=(level:string|undefined)=>level==="HIGH"?"vysoká":level==="MEDIUM"?"střední":level==="LOW"?"nízká":"—";
