...
export function PredictionOffers({ user, marketView }: { user: SessionUser | null; marketView: boolean }) {
  ...
  const groups = useMemo(() => groupCompetitionFixtures(visible.map((offer) => ({ ...offer, leagueLogoUrl: `https://media.api-sports.io/football/leagues/${offer.leagueId}.png` }))), [visible]);
  ...
  return <section className="mt-4">
    ...
    <CompetitionDayTabs days={days} activeDate={activeDate} onSelect={selectDate} />
    ...
    <div className="mt-3 space-y-3">
      {groups.map((group, index) => {
        const open = expandedLeagues.has(group.leagueId);
        const showKind = group.kind !== groups[index - 1]?.kind;
        return <div key={group.leagueId}>
          ...
          <section className="ui-panel overflow-hidden">
            <div className="flex min-h-14 items-center gap-2 px-3.5 py-2.5">
              <button type="button" aria-expanded={open} onClick={() => toggleLeague(group.leagueId)} className="flex min-h-11 min-w-0 flex-1 items-center gap-2 text-left">
                <TeamLogo src={group.logoUrl} alt={group.name} size={20} /><strong className="truncate text-sm">{group.name}</strong><span className="text-xs text-muted">{group.fixtures.length}</span><span className="ml-auto text-muted" aria-hidden>{open ? "▲" : "▼"}</span>
              </button>
              <Link href={`/?date=${activeDate}&league=${group.leagueId}`} className="ui-chip min-h-11 px-3 text-xs">Program</Link>
      ...
      {!groups.length && <p className="rounded-xl border border-border bg-surface p-5 text-sm text-muted">Pro zvolený den a filtry nemáme uloženou predikci.</p>}
    </div>
  </section>;
}
