...
import { TodaySelections } from "./TodaySelections";
...
export function ModelLab({ isPro, isAdmin }: { isPro: boolean; isAdmin: boolean }) {
  ...
  return <section className="rounded-2xl border border-border bg-surface p-4 shadow-[var(--shadow-panel)] sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="page-kicker">Centrum ověřování</p><h2 className="mt-1 text-2xl font-bold text-foreground">Model Lab</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-muted">Co model skutečně ověřil, co teprve sbírá data a které dnešní výběry splnily pevná pravidla.</p></div><Tabs items={[{ value: "LEAGUE", label: "Ligy" }, { value: "EURO_CUP", label: "Evropa" }, { value: "NATIONAL", label: "Reprezentace" }]} value={context} onChange={setContext} label="Kontext modelů" /></div>
    <TodaySelections enabled={isPro} />
    <div className="mt-5 flex items-end justify-between gap-3"><div><p className="page-kicker">Strategie</p><h3 className="mt-1 text-lg font-bold">Stav modelů</h3></div><p className="text-xs text-muted">Kliknutím otevřeš audit a aktuální výběry.</p></div>
...
function Detail({ card, isPro, isAdmin, context, detail, activity, activityError, onStatus }: { card: Card; isPro: boolean; isAdmin: boolean; context: string; detail: Payload | null; activity: Activity | null; activityError: boolean; onStatus: (status: string) => void }) {
  ...
  return <div className="mt-3 space-y-3"><ActivityPanel card={card} isPro={isPro} activity={activity} error={activityError} context={context} />
    <section><h4 className="text-xs font-bold">Strategie a portfolio</h4><div className="mt-2 grid grid-cols-3 gap-2"><Mini label="Bilance" value={`${s.portfolio.hits}/${s.portfolio.settled}`} /><Mini label="Holdout ROI" value={pct(s.holdout.roi)} /><Mini label="Max. propad" value={`${s.portfolio.maxDrawdown.toFixed(1)} u`} /></div>{s.portfolio.roiConfidence95 && <p className="mt-2 text-[10px] leading-4 text-muted">95% interval ROI {pct(s.portfolio.roiConfidence95.low)} až {pct(s.portfolio.roiConfidence95.high)}. Kladné krátkodobé ROI samo nepotvrzuje sázkovou výhodu.</p>}</section>
