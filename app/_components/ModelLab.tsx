...
type Card = { strategy: string; policyVersion: number; title: string; status: string; minimumSample: number; rules: string; modelContext: string; summary: Summary; research?: { n: number; mae: number | null; bias: number | null; version: number | null } | null };
...
export function ModelLab({ isPro, isAdmin }: { isPro: boolean; isAdmin: boolean }) {
  ...
  function open(card: Card) {
    ...
    if (isPro) fetch(`/api/picks/model-lab?context=${context}&strategy=${card.strategy}&detail=true`, { cache: "no-store" }).then(async (response) => response.ok && setDetail(await response.json() as Payload));
  }
...
function Segments({ payload }: { payload: Payload }) { return <section><h4 className="text-xs font-bold">Segmenty a konkrétní zápasy</h4><div className="mt-2 space-y-2">{payload.segments?.map((segment) => <details key={segment.kind} className="rounded-lg border border-border bg-background"><summary className="cursor-pointer px-3 py-2 text-xs font-semibold">{segment.kind}</summary><div className="border-t border-border p-2">{segment.groups.map((group) => <p key={group.label} className="flex justify-between gap-3 py-1 text-[11px]"><span>{group.label}{group.descriptiveOnly ? " · popisné" : ""}</span><span>{group.portfolio.settled} · ROI {pct(group.portfolio.roi)} · CLV {pct(group.portfolio.averageClv)}</span></p>)}</div></details>)}</div><p className="mt-2 text-[10px] text-muted">Načteno {payload.detailRows?.length ?? 0} posledních ledger záznamů.</p></section>; }
