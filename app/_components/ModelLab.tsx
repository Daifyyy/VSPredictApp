...
import { LiveModelLab } from "./LiveModelLab";
...
function TodaySelections({ enabled }: { enabled: boolean }) {
  const [payload, setPayload] = useState<{ rows: TodayRow[]; correlatedFixtures: number[] } | null>(null);
  useEffect(() => {
    if (!enabled) return;
    const controller = new AbortController();
    fetch("/api/picks/today", { cache: "no-store", signal: controller.signal }).then(async (response) => { if (response.ok) setPayload(await response.json()); }).catch(() => undefined);
    return () => controller.abort();
  }, [enabled]);
  if (!enabled) return <p className="mt-4 rounded-xl border border-border bg-background p-3 text-xs text-muted"><strong className="text-foreground">Dnešní výběry</strong> · konkrétní ledger a překryvy jsou součástí PRO.</p>;
  if (!payload) return <div className="mt-4 h-16 animate-pulse rounded-xl bg-border/50 motion-reduce:animate-none" />;
  return <section className="mt-4 rounded-xl border border-border bg-background p-3"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-bold">Dnešní výběry</h3><p className="mt-0.5 text-[10px] text-muted">Autonomní 1u, výzkumné signály a tvoje tipy na jednom místě.</p></div><span className="rounded-full bg-accent/20 px-2 py-1 text-[10px] font-bold">{payload.rows.length}</span></div>{payload.rows.length ? <div className="mt-2 divide-y divide-border">{payload.rows.map((row) => <div key={`${row.kind}:${row.id}`} className="flex flex-wrap items-center gap-x-3 gap-y-1 py-2 text-[11px]"><time className="font-semibold text-muted">{new Date(row.kickoff).toLocaleTimeString("cs-CZ", { timeZone: "Europe/Prague", hour: "2-digit", minute: "2-digit" })}</time><strong>{row.homeName} – {row.awayName}</strong><span>{row.label} · {row.side}{row.line == null ? "" : ` ${row.line.toFixed(1)}`}</span>{row.probability != null && <span>model {(row.probability * 100).toFixed(0)} %</span>}<span>{row.price == null ? "bez zmrazené ceny" : `kurz ${row.price.toFixed(2)}${row.bookmaker ? ` · ${row.bookmaker}` : ""}`}</span><span className={`ml-auto rounded-full px-2 py-0.5 font-bold ${row.kind === "AUTONOMOUS" ? "bg-positive/10 text-positive" : row.kind === "MANUAL" ? "bg-accent/25" : "bg-warning/10 text-warning"}`}>{row.kind === "AUTONOMOUS" ? "Simulace 1u" : row.kind === "MANUAL" ? "Můj tip" : "Výzkum"}</span>{payload.correlatedFixtures.includes(row.fixtureId) && <span className="w-full text-right text-[10px] text-warning">Korelovaná expozice: tento zápas má více souvisejících trhů.</span>}</div>)}</div> : <p className="mt-2 text-xs text-muted">Dnes zatím není žádný zmrazený výběr ani výzkumný signál.</p>}</section>;
}
...
function ActivityEntry({ row, context, current }: { row: ActivityRow; context: string; current: boolean }) {
  ...
  const selection = row.market === "1X2" ? row.side === "HOME" ? row.homeName : row.awayName : row.market === "OVER_25" ? "Over 2,5 gólu" : row.market === "CORNERS" ? `${row.side === "OVER" ? "Over" : "Under"} ${row.line?.toFixed(1) ?? "—"} rohů` : "Oba týmy skórují · Ano";
