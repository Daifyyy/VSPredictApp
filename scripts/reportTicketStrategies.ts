...
for (const strategy of ["VALUE", "ELO_INTUITION"] as const) {
  ...
  console.log(JSON.stringify({ strategy, settledTickets:tickets.length, nextCheckpoint:checkpoint, ticketHitRate:tickets.length?tickets.filter(t=>t.hit).length/tickets.length:null, legHitRate:legs.length?legs.filter(l=>l.hit).length/legs.length:null, roi:priced.length?priced.reduce((s,t)=>s+t.profit!,0)/priced.length:null, averageOdds:priced.length?priced.reduce((s,t)=>s+t.combinedOdds!,0)/priced.length:null, logLoss, calibrationEce, clv:clvRows.length?clvRows.reduce((a,b)=>a+b,0)/clvRows.length:null, clvSample:clvRows.length, note:"Strategie se automaticky nepovyšuje ani neslučuje; rozhodnutí patří až reportu 200 oceněných tiketů."}));
}}
